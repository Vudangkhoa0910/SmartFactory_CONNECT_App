...
class IdeaBoxItem {
  ...
  factory IdeaBoxItem.fromJson(Map<String, dynamic> json) {
    ...
    DifficultyLevel? parseDifficulty(dynamic score) {
      ...
      final int val = score is int ? score : int.tryParse(score.toString()) ?? 0;
    ...
    IssueType parseIssueType(String? category) {
      switch (category) {
        case 'quality_improvement': return IssueType.quality;
        case 'safety_enhancement': return IssueType.safety;
        case 'productivity': return IssueType.performance;
        case 'cost_reduction': return IssueType.energySaving; // Mapping energy to cost/env
        case 'environment': return IssueType.energySaving;
        case 'process_improvement': return IssueType.process;
        case 'workplace': return IssueType.workEnvironment;
        ...
        case 'welfare': return IssueType.welfare; // If backend adds this
        case 'pressure': return IssueType.pressure;
        case 'psychological_safety': return IssueType.psychologicalSafety;
        case 'fairness': return IssueType.fairness;
    ...
    IdeaStatus parseStatus(String? status) {
      switch (status) {
        case 'submitted':
        case 'pending': return IdeaStatus.submitted;
        case 'under_review': return IdeaStatus.underReview;
        case 'escalated': return IdeaStatus.escalated;
        case 'approved': return IdeaStatus.approved;
        case 'rejected': return IdeaStatus.rejected;
        case 'implementing': return IdeaStatus.implementing;
        case 'completed':
        case 'implemented': return IdeaStatus.completed;
        default: return IdeaStatus.submitted;
      }
    }
    ...
    List<IdeaProcessLog> parseProcessLogs(Map<String, dynamic> json) {
      ...
      if (json['history'] != null && json['history'] is List) {
        for (var h in json['history']) {
          ...
          switch (h['action']) {
            case 'submitted': status = IdeaStatus.submitted; comment = 'Đã gửi góp ý'; break;
            case 'assigned': status = IdeaStatus.underReview; comment = 'Đã phân công xử lý'; break;
            case 'reviewed': 
              // Parse details for status
              try {
                final details = h['details'] is String ? jsonDecode(h['details']) : h['details'];
              ...
              break;
            case 'implemented': status = IdeaStatus.implementing; comment = 'Đã triển khai'; break;
            case 'escalated': 
              ...
              break;
            default: status = IdeaStatus.underReview; comment = h['action'];
          }

          logs.add(IdeaProcessLog(
            id: h['id'].toString(),
      ...
      if (json['responses'] != null && json['responses'] is List) {
        for (var r in json['responses']) {
          logs.add(IdeaProcessLog(
            ...
            statusChange: IdeaStatus.underReview, // Responses don't necessarily change status
            comment: r['response'] ?? '',
          ));
        }
      }
    ...
    return IdeaBoxItem(
      ...
      currentHandlerName: json['assigned_to_name'] ?? json['currentHandlerName'],
